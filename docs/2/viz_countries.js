...
function vizCountries(data, maxValue, cb)	{
	...
	var scaleChina = d3.scaleLinear()
										.domain([0,maxChina])
										//.range([0, d3.min([ bb.width - 200, maxChina ]) ]);
										.range([0, d3.min([ bb.width/2 - 200, maxChina ]) ]);

	var scaleWorld = d3.scaleLinear()
										.domain([0,maxWorld])
										.range([0, d3.min([ 400, maxWorld ]) ]);
	...
	v.enter()
		...
		.merge(v)
			.styles({
				left	:d=>d.bb.x+'px',
				top		:d=>d.bb.y+'px',
				width	:d=>d.bb.width+'px',
				height:d=>d.bb.height+'px',
			})
			.call(sel=>{
				...
				svg.enter()
					...
					.merge(svg)
						.attrs({
							viewBox:d=>[0,0,d.bb.width, d.bb.height].join(' '),
							overflow:'hidden',
						})
						.call(sel=>{
